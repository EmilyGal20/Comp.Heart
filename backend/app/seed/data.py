...
def seed_database(db: Session):
    ...
    organizations = [
        Organization(
            ...
            description="Multi-site healthcare operations and support workflows.",
        ),
        Organization(
            name="Aether Labs",
            slug="aether",
            ...
            description="Platform engineering, AI operations, and release management.",
        ),
        Organization(
            ...
            description="Omnichannel retail and fulfillment operations.",
        ),
    ]
    ...
    tag_names = ["incident", "sla", "process", "release", "automation", "onboarding", "retail", "ops"]
    tags = {name: KnowledgeTag(name=name) for name in tag_names}
    db.add_all(tags.values())
    db.flush()

    knowledge_items = [
        KnowledgeItem(
            title="Northstar Incident Command Runbook",
            slug="northstar-incident-command-runbook",
            category="process",
            organization_id=organizations[0].id,
            content="Acknowledge critical issues in 15 minutes, assign a commander, open a bridge, update stakeholders, and log patient-impact decisions.",
            summary="The first-hour response flow for critical healthcare operations incidents.",
            author_id=users[2].id,
            is_published=True,
            tags=[tags["incident"], tags["sla"], tags["process"]],
        ),
        KnowledgeItem(
            title="Aether Release Readiness Checklist",
            slug="aether-release-readiness-checklist",
            category="operations",
            organization_id=organizations[1].id,
            content="Verify rollback owner, feature flags, telemetry monitors, communication plan, and automation rule health before each release.",
            summary="Release-readiness guardrails for AI platform deployments.",
            author_id=users[4].id,
            is_published=True,
            tags=[tags["release"], tags["automation"], tags["ops"]],
        ),
        KnowledgeItem(
            title="Harbor Store Systems Rollout Playbook",
            slug="harbor-store-systems-rollout-playbook",
            category="operations",
            organization_id=organizations[2].id,
            content="Coordinate store training, cutover timing, escalation coverage, and post-launch verification for every rollout wave.",
            summary="How Harbor launches internal systems across store regions safely.",
            author_id=users[8].id,
            is_published=True,
            tags=[tags["retail"], tags["process"], tags["ops"]],
        ),
        KnowledgeItem(
            title="Aether New Teammate Operating Guide",
            slug="aether-new-teammate-operating-guide",
            category="people",
            organization_id=organizations[1].id,
            content="Review task lifecycle, release rituals, incident channels, and AI escalation norms in week one.",
            summary="Onboarding expectations for Aether Labs team members.",
            author_id=users[4].id,
            is_published=True,
            tags=[tags["onboarding"], tags["process"]],
        ),
    ]
    ...
    tasks = [
        Task(
            title="Stabilize patient billing escalation lane",
            description="Investigate delayed claim notifications and tighten the support escalation workflow.",
            organization_id=organizations[0].id,
            status="in_progress",
            priority="high",
            ...
            sla_hours=8,
            sla_status="breached",
            related_knowledge_id=knowledge_items[0].id,
        ),
        Task(
            title="Audit ICU workflow announcements",
            description="Review org-wide messaging for high-severity operational updates and update templates.",
            organization_id=organizations[0].id,
            status="review",
            priority="medium",
        ...
        Task(
            title="Prepare release command center",
            description="Confirm owners, monitoring, rollback paths, and AI escalation prompts for Friday deployment.",
            organization_id=organizations[1].id,
            status="in_progress",
            priority="critical",
            ...
            sla_hours=6,
            sla_status="breached",
            related_knowledge_id=knowledge_items[1].id,
        ),
        Task(
            title="Tune automation severity mapping",
            description="Refine the threshold logic used for manager-facing automation alerts.",
            organization_id=organizations[1].id,
            status="todo",
            priority="medium",
            assignee_id=users[6].id,
        ...
        Task(
            title="Coordinate west region store rollout",
            description="Align training schedule, issue coverage, and overnight support for the next launch wave.",
            organization_id=organizations[2].id,
            status="in_progress",
            priority="high",
            assignee_id=users[9].id,
            creator_id=users[8].id,
            due_at=now + timedelta(hours=16),
            sla_hours=20,
            sla_status="warning",
            related_knowledge_id=knowledge_items[2].id,
        ),
        Task(
            title="Refresh systems coordinator checklist",
            description="Update task and communication checklists for the store systems coordinator role.",
            organization_id=organizations[2].id,
            status="todo",
            priority="low",
            assignee_id=users[9].id,
    ...
    rules = [
        AutomationRule(
            name="Northstar Overdue SLA Alert",
            description="Escalates overdue support tasks to the manager and notification center.",
            organization_id=organizations[0].id,
            trigger_type="task.updated",
            condition_json='{"sla_status": "breached"}',
            action_json='{"type": "notify", "audience": "managers", "severity": "critical"}',
            is_enabled=True,
        ),
        AutomationRule(
            name="Aether High Priority Release Ping",
            description="Notifies automation leadership when critical release tasks appear.",
            organization_id=organizations[1].id,
            trigger_type="task.created",
            condition_json='{"priority": "critical"}',
            action_json='{"type": "notify", "audience": "managers", "severity": "high"}',
            is_enabled=True,
        ),
        AutomationRule(
            name="Harbor Rollout Playbook Boost",
            description="Boosts retail rollout knowledge in the AI assistant.",
            organization_id=organizations[2].id,
            trigger_type="knowledge.updated",
            condition_json='{"category": "operations"}',
            action_json='{"type": "ai_suggest", "weight": "boost"}',
            is_enabled=True,
        ),
    ]
    db.add_all(rules)
