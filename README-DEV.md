# CompHeart — Development

Back to the [main README](README.md) · [Production deployment](README-PROD.md)

This document covers **local development**: installation, how to run the stack, environment files, the optional native risk helper, demo accounts, the full API list, and product detail that was previously in a single long README.

## Super admin & admin: Settings and passwords

- **Settings (super admin):** Admin shortcuts, a live **platform snapshot** (from `GET /api/admin/global-summary`), and **organization defaults** (SLA, approvals, recurring, notification toggles) when an organization is selected in the top bar. **Admins (non–super admin)** see org defaults for their own company without selecting scope.
- **Change your own password:** **Settings → Change password** — requires current password. API: `PUT /api/settings/password` with `current_password` and `new_password` (min 6 characters).
- **Set another user’s password:** **People** page — **Set password** (super admin and org admin only; not managers). API: `PATCH /api/users/{id}/password` with `new_password`. Only a super admin may set a password for another **super admin** user.
...
`npm run dev` starts Vite with **same-origin** `/api` and `/ws/*` proxies to the API on 7155, so the browser does not need CORS for the typical setup. The dev server is bound to **all interfaces** (`host: true`); to demo from another device on the same network, use `http://<this-machine-LAN-IP>:8069`. If you run a **pre-built** static frontend and point it at a separate API host, set `VITE_API_BASE_URL` at build time and add matching origins in the backend `CORS_EXTRA_ORIGINS` (see `backend/.env.example`).
...
- `emily@compheart.com` (`SUPER_ADMIN`)
- `maya@northstar.com` (`ADMIN`)
- `ava@northstar.com` (`MANAGER`)
- `leo@northstar.com` (`USER`)
- `jordan@aether.com` (`ADMIN`)
- `noah@aether.com` (`MANAGER`)
- `iris@aether.com` (`USER`)
- `camila@harbor.com` (`ADMIN`)
- `marcus@harbor.com` (`MANAGER`)
- `nina@harbor.com` (`USER`)
...
- `POST /api/users`
- `PUT /api/users/{id}`
- `PATCH /api/users/{id}/password` (admin / super admin; body: `new_password`)
...
- `PUT /api/settings/password` (own password: `current_password`, `new_password`)
- `GET /api/settings/workspace`
- `PUT /api/settings/workspace`
- `GET /api/settings/organization` (optional `?organization_id=` for super admins)
- `PUT /api/settings/organization` (optional `?organization_id=` for super admins)
